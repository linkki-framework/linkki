...
class HierarchicalTableUtilTest {
    ...
    void testExpandNodesIf() {
        ...
        HierarchicalTableUtil.expandNodesIf(grid, 3, this::testFilter);

        assertThat(grid.isExpanded(rootItem))
                .as("Root item should be expanded as the filter returns true")
                .isTrue();
        var firstLevelItem = getRootItem(grid).getChildRows().get(0);
        assertThat(grid.isExpanded(firstLevelItem))
                .as("First level item should NOT be expanded as the filter returns false")
                .isFalse();
        var secondLevelItem = firstLevelItem.getChildRows().get(0);
        assertThat(grid.isExpanded(secondLevelItem))
                .as("Second level item should NOT be expanded as the filter returns false")
                .isFalse();
        var thirdLevel = secondLevelItem.getChildRows().get(0);
        assertThat(grid.isExpanded(thirdLevel))
                .as("Third level item should NOT be expanded as the filter returns false")
                .isFalse();
    }
    ...
    private Boolean testFilter(TestTreeTableRowPmo rowPmo) {
        return rowPmo.remainingLevels > 2;
    }
