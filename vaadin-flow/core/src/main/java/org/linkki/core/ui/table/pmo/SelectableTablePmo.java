...
/**
 * PMO for a table whose rows can be selected. In addition, a double click action can be bound to
 * the rows.
 */
@BindTableSelection
public interface SelectableTablePmo<ROW> extends ContainerPmo<ROW> {
     ...
     *           {@link #setSelection(Object)} has already been called before this method is
     *           invoked. Thus, it is safe to assume that the row that is being clicked on is
     *           identical to {@link #getSelection()}.
     */
    void onDoubleClick();
}
