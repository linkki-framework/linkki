...
import org.linkki.samples.playground.ts.table.NumberFooterTablePmo.NumberFooterRowPmo;
...
public class NumberFooterTablePmo implements ContainerPmo<NumberFooterRowPmo> {
    ...
    private final List<NumberFooterRowPmo> items;

    public NumberFooterTablePmo() {
        var random = new Random(42);
        this.items = IntStream.range(0, 11)
                .mapToObj(i -> new NumberFooterRowPmo(random.nextDouble() * 10))
                .toList();
    ...
    public static class NumberFooterRowPmo {
        ...
        public NumberFooterRowPmo(double value) {
            this.decimal = Decimal.valueOf(value);
        }
