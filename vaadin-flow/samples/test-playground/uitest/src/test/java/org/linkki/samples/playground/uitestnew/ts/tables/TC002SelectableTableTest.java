...
class TC002SelectableTableTest extends PlaygroundUiTest {
    ...
    void testSelection() {
        ...
        assertThat("The selected row in the PMO should be the same as the selected row in the table",
                   ...
                   allOf(is("Name 2"),
                         is($(LinkkiTextElement.class).id(SelectionComparisonSectionPmo.PROPERTY_TABLE_SELECTION)
                                 .getText())));
    ...
    void testAdd_NewRowSelected() {
        ...
        LinkkiSectionElement section = $(LinkkiSectionElement.class).withId("PlaygroundSelectableTablePmo").single();
        section.$(ButtonElement.class).withId("add").single().click();
