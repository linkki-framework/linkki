...
`BusinessPartner`:: A spring specific class which allows our project to be run as a Java application.
This class will not be found in the plain archetype.
...
`HelloPmo`:: Defines a section on the webpage which is used in the UI of the application.
You can apply annotations like `@UITextfield` to your methods to create specific UI elements.
For more information about creating a UI see <<ui-element, UI elements>>.
