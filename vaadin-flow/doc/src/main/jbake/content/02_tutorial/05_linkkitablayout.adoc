...
[NOTE]
If you look at the implementation of `BusinessPartnerView`, you will notice the https://vaadin.com/docs/v24/flow/routing/route[`@Route`] annotation.
...
Therefore, you need to add a https://vaadin.com/docs/v24/flow/routing/route-parameters[parameter] to the URL.

One way to achieve this is to have the view implement the interface https://vaadin.com/docs/v24/flow/routing/route-parameters/#defining-route-parameters[`HasUrlParameter<String>`] and override the method `setParameter(BeforeEvent event, @OptionalParameter String parameter)`.
