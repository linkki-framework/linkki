...
Therefore, you will add an additional tab sheet to handle error cases: If the UUID is missing or incorrect, the first tab will be shown and the other two will be hidden. Otherwise, the first tab will be hidden and the other two will display detailed information on the `BusinessPartner`.
...
If you change the UUID in the URL to some unknown value, for example http://localhost:8080/PartnerDetails/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa, you should get only one tab. This tab should have a warning icon and display "No partner could be found with the given ID".
