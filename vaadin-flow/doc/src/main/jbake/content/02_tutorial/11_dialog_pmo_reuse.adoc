...
Now, we add a new class `AddAddressDialogPmo`, which inherits from <<ui-open-dialog-button, `DialogPmo`>>.
...
However, using <<ui-open-dialog-button, `@UIOpenDialogButton`>> is the preferred way for creating dialogs on button press.
