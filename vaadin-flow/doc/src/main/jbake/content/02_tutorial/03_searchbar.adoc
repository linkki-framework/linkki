...
. Create a constructor in `SearchPage` with an argument of type `org.linkki.samples.appsample.model.BusinessPartnerRepository`. This type is provided by the dependency `sample-model` and implements the search functionality.
