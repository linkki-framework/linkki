...
[NOTE]
You do not need to use `AbstractPage`. Here, it is convenient because it will convert your PMOs to Vaadin components. Instead, you could also use a layout such as <<howto-using-pmos-in-layouts, `VerticalLayout`>>.
...
. Create a constructor in `SearchPage` with an argument of type `org.linkki.samples.appsample.model.BusinessPartnerRepository`. This type is provided by the dependency `tutorial-model` and implements the search functionality.
