...
.. Style the button by setting the attribute `variants` to `{ButtonVariant.LUMO_PRIMARY, ButtonVariant.LUMO_SMALL}`. Just like the icon, the `ButtonVariant` is a https://vaadin.com/docs/v24/components/button[Vaadin component].
