...
The linkki Karibu module further simplify the set-up with the utility class `KaribuUI` and the JUnit extension `KaribuUIExtension`.
...
[[karibu-ui]]
==== Explicit set-up using `KaribuUI`

With `KaribuUIExtension`, the set-up must be the same for all methods in the test class as the set-up is called implicitly in the extension methods.
In some cases, the set-up needs to be called explicitly, e.g. if the set-up differs for each method, or the implementer wants to only set up and tear down once per class.

In these cases, `KaribuUI#setUp` can be used directly without an extension.
As tear-down is not automatically executed by an extension, caller must make sure to call `KaribuUI#tearDown` afterwards explicitly.

[NOTE]
====
If additional set-up is needed in your project, you can create your own utility class such as `MyProjectTestUI` that provides set-up and tear-down methods which delegates to `KaribuUI`.
====

.Set-up with KaribuUI
[source,java]
----
import com.vaadin.flow.component.UI;
import java.util.Locale;

class SetupWithKaribuUI {

    @Test
    void testUI() {
        KaribuUI.setup(KaribuConfiguration.withDefaults()
...
[[karibu-limitations]]
==== Limitations of Karibu

Karibu-Testing is intentionally designed as a *server-side, browserless* testing framework for Vaadin Flow applications.
It runs entirely inside the JVM and interacts directly with server-side constructs such as `UI`, `View`, and Vaadin components, rather than communicating with a real browser.

More information can be found on https://github.com/mvysny/karibu-testing/blob/master/README.md[Karibu's GitHub page].

===== No Client-Side Testing

Karibu does not execute any client-side code.
As a result, it cannot test behavior that depends on the browser or frontend runtime.
Here is a short list of the most relevant limitations:

* JavaScript execution
* Browser DOM behavior
* CSS rendering
* Browser startup and lifecycle
* Static resources
* Cookies or browser-managed session behavior
* HTTP-based routing

For these scenarios, browser-based testing tools (such as end-to-end or UI testing frameworks) should be used instead.

===== Test Scope

Since Karibu-Testing does not run against a live deployment, functionalities that depend on external systems cannot be tested directly.
To cover such cases, it may be necessary to create mock or test-specific implementations when testing views with these dependencies.