...
Consider using `$(OkCancelDialogElement.class)` instead of `$(DialogElement.class)` wherever applicable.
...
* *Disabled input fields not changing their background color* +
Input fields did not turn to a lighter gray when disabled.
// https://jira.convista.com/browse/LIN-4809
* *`OkCancelDialogElement` and `VaadinElementConditions#dialogDisplayed` fixed for multiple dialogs* +
...
* *Card section theme not working in Dialogs* +
The card section theme was no longer working for PMOs and nested PMOs in dialogs.

// https://jira.convista.com/browse/LIN-4805
* *Horizontal LinkkiTabLayouts getting padding when nested inside a vertical LinkkiTabLayout* +
...
* *Dialogs not rendered on top of each other* +
