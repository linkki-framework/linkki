...
* *Spring Boot 4* +
Applications using Spring must upgrade to *Spring Boot 4* and *Spring Framework 7*.
...
* *JakartaEE 11* +
Applications using Spring must upgrade to *JakartaEE 11*.
This is a major version jump from JakartaEE 10.
* *Node.js* +
Frontend builds now require *Node.js 24 or later* for development.
* *JUnit* +
All linkki test libraries now require *JUnit 6*.
The migration should be straightforward, without noticeable API changes.
For all changes and improvements, see https://docs.junit.org/6.0.2/release-notes.html[release notes for JUnit 6].
* *Selenium* +
...
For <<apply-linkki-theme, linkki and the F10 theme>>, a new `STYLESHEET` constant was added to `LinkkiTheme` and `F10ProductTheme` that can be used with `@Stylsheet` to replace `@Theme`.
