...
For the <<apply-linkki-theme, linkki theme>> and the <<apply-f10-theme, F10 theme>>, a new `STYLESHEET` constant was added to `LinkkiTheme` and `F10ProductTheme`. Replace existing `@Theme` declarations with `@StyleSheet(LinkkiTheme.STYLESHEET)` or `@StyleSheet(F10ProductTheme.STYLESHEET)`.
