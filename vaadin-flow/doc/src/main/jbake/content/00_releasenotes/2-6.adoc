...
New converters have been added to convert `GregorianCalendar` and `Money` to `String`.
...
In the Faktor-IPS extension, a new converter for `Money` has been added which can be used for `String`-valued fields such as `@UITextField`. Its functionality is documented <<ips-converters, here>>. This converter is not applied by default, thus have to be added to the converter registry if needed.


//https://jira.convista.com/browse/LIN-3680
===== New FormattedStringToNumberConverters

The old `FormattedNumberToStringConverter`s have been deprecated and replaced with new ones that match the correct naming schema.
...
The new converters now also check if the input value is too large and return an error if that's the case.

The linkki UI components now use the new converters by default.
