...
| Spring Boot                   | 3.4.4 (link:https://github.com/spring-projects/spring-boot/wiki/Spring-Boot-3.4-Release-Notes[Release Notes]) | 3.4.3
|===
...
* Required fields (e.g., `TextField`, `ComboBox`) were not marked red when prefilled with an invalid value under certain conditions. This was a Vaadin bug that is fixed with this version. Note that the issue only appeared in 2.7.1 and does not affect 2.7.0.
