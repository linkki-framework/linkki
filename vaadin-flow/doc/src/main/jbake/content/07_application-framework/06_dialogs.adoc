...
* An existing dialog that was not created with a `DialogPmo` should be reused
* The dialog should only be shown under certain conditions — the annotated method may return `null` in this case, which still triggers a binding context update

.Dialog created with `@UIOpenDialogButton` using `Function<Handler, OkCancelDialog>`
[source,java]
----
include::{source-dir}/org/linkki/samples/playground/ts/dialogs/UIOpenDialogButtonPmo.java[tags=uiOpenDialogButtonWithFunction]
----