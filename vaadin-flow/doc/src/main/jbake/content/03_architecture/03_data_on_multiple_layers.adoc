...
For situations where a method annotated with `@ModelObject` returns `null`, the fields are treated as read only as long as no model object exists. As soon as one is available, methods from model object are called accordingly.