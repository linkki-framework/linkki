...
* `StaticValueDispatcher` provides information that can be read directly from UI annotations
...
Custom dispatchers can be created via a `PropertyDispatcherFactory`. There, the method `createCustomDispatchers` can be overridden to create one or more custom dispatchers. These dispatchers are inserted between `BehaviorDependentDispatcher` and `StaticValueDispatcher`.
