...
With linkki 2.4.0 we introduced the class `LumoConstants` because Vaadin decided to not use real constant definitions in `LumoUtility` anymore.
With Vaadin 24.1.2 they reverted the change, so we can use `LumoUtility` directly again. As a result `LumoConstants' is now deprecated.
