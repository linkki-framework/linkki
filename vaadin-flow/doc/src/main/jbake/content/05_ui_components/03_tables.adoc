...
the `ContainerPmo` implementation should also implement the interface `SelectableTablePmo`. This interface requires three methods:
...
However, in scenarios where selection is needed for visual reasons only, the `@BindTableSelection(visualOnly = true)` annotation can be applied. This allows tables to be made selectable without the need to implement the `SelectableTablePmo` interface.
