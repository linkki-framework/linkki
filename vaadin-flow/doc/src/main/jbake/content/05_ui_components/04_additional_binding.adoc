...
| `<<bind-helper-text, @BindHelperText>>` | Set helper text on UI elements using default `HelperTextType.AUTO`
| `<<bind-height, @BindHeight>>` | Set height of a component
| `<<bind-width, @BindWidth>>` | Set width of a component
| `<<bind-clear-button, @BindClearButton>>` | Set clear button visibility on UI components
| `<<bind-value-change-mode, @BindValueChangeMode>>` | Set `ValueChangeMode` of a component
| `<<bind-form-item-label-width, @BindFormItemLabelWidth>>` | Set label width of all form items of a layout
|===
...
TIP: If a component is supposed to be writable even though the rest of the UI is in read-only-mode, use <<bind-read-only-behavior,`@BindReadOnlyBehavior`>> with value `WRITABLE`, e.g. an input field to filter the content of a table.
...
IMPORTANT: `WRITABLE` can only be used on UI elements which implement `HasValue`.
...
The Vaadin documentation includes a https://vaadin.com/docs/v25/components/icons[reference page listing all available icons].
...
[source,java]
----
include::{source-dir-playground}/org/linkki/samples/playground/ts/table/SimplePlaygroundTablePmo.java[tags=bindPlaceholder-tablePmo]
----
...
The annotation `@BindHelperText` can be used to display a https://vaadin.com/docs/v25/components/text-field#helper[helper text] on UI elements.

It has two properties:

* <<helper-text-value, `value: String`>>
* <<helper-text-type, `helperTextType: HelperTextType`>>

[[helper-text-value]]
Value::
This is the content that is displayed on a UI element as a helper text.

[[helper-text-type]]
HelperTextType::
The following configuration options are available for `HelperTextType`:

[cols="1,9"]
.HelperTextType
|===
|`AUTO`     |The text is read from the attribute `value` if it is not empty, otherwise it reacts like `DYNAMIC` (*default*)
|`STATIC`   |The text is read from the attribute `value`
|`DYNAMIC`  |The text is determined by the return value of the method `String get<PropertyName>HelperText()`. The `value` is ignored.
|===

[[bind-height]]
===== Height

The annotation `@BindHeight` can be used to define the height of a UI component.
It has a single property `value`, which defines the height that should be applied to the UI component.
The value can be any valid CSS width value, such as `"100px"`, `"50%"`, or `"10em"`.

[NOTE]
This annotation can be particularly useful for table-like components such as `UITableComponent`, where the display area often benefits from a fixed height (e.g. to show a consistent number of rows).

[[bind-width]]
===== Width

The annotation `@BindWidth` can be used to define the width of a UI component.
It has a single property `value`, which defines the width that should be applied to the UI component.
The value can be any valid CSS height value, such as `"100px"`, `"50%"`, or `"10em"`.

[[bind-clear-button]]
===== Clear Button

The annotation `@BindClearButton` can be used to make the https://vaadin.com/docs/v25/components/date-picker#basic-features[clear button] visible on UI components like `UITextField`, `UIDateField`, etc.
Clicking on the clear button sets the field value to `null`.

[NOTE]
====
This annotation can only be used with UI components that implement the `HasClearButton` interface.
Therefore, components like `UICheckbox` or `UIRadioButtons` cannot be used with `BindClearButton`.

Additionally, the property getter must return a non-primitive type (e.g., `Integer` instead of `int` for `UIIntegerField`) as otherweise the value cannot be set to `null`.

Note that it does not work with `UIComboBox`, as the visibility of the clear button there is determined by the available values (i.e., whether `null` is allowed).
For `UIMultiSelect`, a clear button is always available.
====

[[bind-value-change-mode]]
===== Value change mode

The annotation `BindValueChangeMode` can be used to set the value change mode of a UI component.
It has a single property `value`, which takes the `ValueChangeMode` that the component should be set to.
For example, it can be useful when applying it on an input field for a search filter.

[Note]
====
This annotation can only be used with UI components that implement `HasValueChangeMode` interface.
Therefore, components like `UICheckbox` or `UIRadioButtons` cannot be used with `BindValueChangeMode`.
====

.`@BindValueChangeMode` on a text field
[source,java]
----
include::{source-dir-playground}/org/linkki/samples/playground/ts/aspects/BindValueChangeModePmo.java[tags=bindValueChangeMode]
----

[[bind-form-item-label-width]]
===== Form-item label width

The annotation `@BindFormItemLabelWidth` can be used to define the label width of form items of a specific layout.
It has a single property `value`, which defines the width that should be applied to the layouts items.
The value can be any valid CSS height value, but anything other unit than `"em"` or `"rem"` is strongly discouraged.
