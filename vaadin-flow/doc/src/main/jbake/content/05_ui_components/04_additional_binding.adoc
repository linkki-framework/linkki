...
The Vaadin documentation includes a https://vaadin.com/components/vaadin-icons/java-examples#all-icons[reference page listing all available icons] as well as documentation how to https://vaadin.com/docs/v24/flow/application/resources#icons[Icons].
...
The annotation `@BindHelperText` can be used to display a https://vaadin.com/docs/v24/components/text-field#helper[helper text] on UI elements.
...
The annotation `@BindClearButton` can be used to make the https://vaadin.com/docs/v24/components/date-picker#basic-features[clear button] visible on UI components like `UITextField`, `UIDateField`, etc.
This annotation can only be used with UI components that implement the `HasClearButton` interface.
Therefore, components like `UICheckbox` or `UIRadioButton` cannot be used with `BindClearButton`.

Clicking on the clear button sets the field value to `null`.
Therefore, the property getter must return a non-primitive type (e.g., `Integer` instead of `int` for `UIIntegerField`).
