...
| `<<bind-height, @BindHeight>>` | `Set height on UI components using the default value "" (no fixed height)`
|===
...
If no value is given (default: `""`), the component keeps its default height.

[NOTE]
This annotation can be particularly useful for table-like components such as `UITableComponent`,
where the display area often benefits from a fixed height (e.g. to show a consistent number of rows).