...
|<<ui-badge,`UIBadge`>> |Badge for displaying visually highlighted information
|===
...
|<<ui-open-dialog-button, `UIOpenDialogButton`>> |A button that opens a dialog. Upon OK click of the dialog, the binding context of the button is updated. (Only available with <<maven-dependency-application-framework,linkki-application-framework-vaadin-flow>>)
|===
...
The value will then be displayed as soon as the future is completed if https://vaadin.com/docs/v25/flow/advanced/server-push[Server Push] is enabled.
...
The annotation `@UICheckbox` creates a https://vaadin.com/docs/v25/components/checkbox[com.vaadin.flow.component.checkbox.Checkbox] that is bound to a `boolean` property.
...
The annotation `@UICheckboxes` creates a https://vaadin.com/docs/v25/components/checkbox[`com.vaadin.flow.component.checkbox.CheckboxGroup`], which is a group of related binary choices.
...
The annotation `@UITextfield` corresponds to a https://vaadin.com/docs/v25/components/text-field[com.vaadin.flow.component.textfield.TextField] for text entry.
...
The annotation `@UITextArea` corresponds to a https://vaadin.com/docs/v25/components/text-area[com.vaadin.flow.component.textfield.TextArea].
...
The annotations `@UIIntegerfield`, `@UIDoubleField` and `@UILongField` are https://vaadin.com/docs/v25/components/text-field[text fields] for displaying formatted numbers.
...
The annotation `@UIDateField` creates a date input field and is equivalent to https://vaadin.com/docs/v25/components/date-picker[`com.vaadin.flow.component.datepicker.DatePicker`].
...
The annotation `@UITimeField` generates a time input field and corresponds to https://vaadin.com/docs/v25/components/time-picker[`com.vaadin.flow.component.datepicker.TimePicker`].
...
The annotation `@UIDateTimeField` generates a date and time input field and corresponds to https://vaadin.com/docs/v25/components/date-time-picker[`com.vaadin.flow.component.datetimepicker.DateTimePicker`].
...
The annotation `@UIComboBox` allows selection of a value from a list and corresponds to https://vaadin.com/docs/v25/components/combo-box[com.vaadin.flow.component.combobox.ComboBox].
...
[source,java]
----
include::{source-dir}/org/linkki/samples/playground/ts/table/dynamicfields/CarPmo.java[tags=ui-combobox]
----
...
Instead, an https://vaadin.com/docs/v25/styling/themes/lumo/utility-classes#text-alignment[appropriate CSS class] has to be set by `@BindComboBoxItemStyle`.
...
The annotation `@UIMultiSelect` allows selection of multiple values from a list and corresponds to https://vaadin.com/docs/v25/components/multi-select-combo-box[com.vaadin.flow.component.combobox.MultiSelectComboBox].
...
* `autoExpand: MultiSelectComboBox.AutoExpandMode` — Controls the automatic expansion of the field when many chips are selected.
The default is `NONE`.
See the https://vaadin.com/docs/v25/components/multi-select-combo-box#auto-expand[Vaadin documentation] for available values.
* `selectedItemsOnTop: boolean` — Can be set to move selected items to the top of the dropdown list.
...
The annotation `@UIRadioButtons` allows selection of a single value using a group of buttons and corresponds to https://vaadin.com/docs/v25/components/radio-button[com.vaadin.flow.component.radiobutton.RadioButtonGroup].
...
[source,java]
----
include::{source-dir}/org/linkki/samples/playground/ts/table/dynamicfields/CarPmo.java[tags=ui-dynamic-field]
----
...
The annotation `@UIBadge` creates a badge for displaying visually highlighted information and corresponds to a badge styled with https://vaadin.com/docs/v25/components/badge[Vaadin's badge theme variants].

The annotated method must return a value that is convertible to `String` with the <<itemCaptionProvider,`ItemCaptionProvider`>> defined in the annotation.

Severity::
The attribute `severity` determines the badge's color scheme (background and text) based on semantic meaning.
Available values are `INFO`, `SUCCESS`, `WARNING`, `ERROR`, and `NONE` (default).
Each severity applies a corresponding color scheme.

Variants::
The attribute `variants` applies additional styling modifications to the badge.
Multiple variants can be combined by providing a String array.
linkki provides the `BadgeVariant` class with common Vaadin badge variants:
 +
* `BadgeVariant.PILL`: Rounded badge shape (default)
* `BadgeVariant.SMALL`: Compact size
* `BadgeVariant.PRIMARY`: Applies primary theme color

[[ui-button]]
==== UIButton

The annotation `@UIButton` is used to mark the method that should be executed when the button is clicked.
It corresponds to https://vaadin.com/docs/v25/components/button[com.vaadin.flow.component.button.Button].
...
For more information please refer to https://vaadin.com/docs/v25/components/button[Vaadin Button Variants Documentation].
...
A menu item should display a caption and must either define a command that is executed when clicked or contain a list of submenu items.
