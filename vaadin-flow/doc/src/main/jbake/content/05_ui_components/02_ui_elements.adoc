...
----
A common usecase is to display a string split accross multiple lines. This can be achieved by using the `HtmlContent.multilineText(String ...)` method.
...
[[ui-datefield]]
==== UIDateField
The annotation `@UIDateField` generates a date input field and corresponds to https://vaadin.com/docs/latest/components/date-picker[`com.vaadin.flow.component.datepicker.DatePicker`]. It allows specifying a date format using the property `dateFormat: String`. If no date format is defined, the format matching the `Locale` setting is used. *linkki* uses `"dd.MM.yyyy"` by default for German. For other languages `DateFormat.SHORT` from the JDK is used.
