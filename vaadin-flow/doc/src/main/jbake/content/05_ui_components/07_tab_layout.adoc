...
The method `LinkkiTabLayout#updateSheetVisibility()` must be triggered to update the visibility state. The method is automatically called by https://vaadin.com/docs/v24/flow/routing/lifecycle/#afternavigationevent[Vaadin's AfterNavigationEvent], that means the visibility state is at least updated when there is a navigation to the view containing the tabs.
