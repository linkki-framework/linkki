...
For general information, see the link:https://vaadin.com/docs/v25/styling/stylesheets[Vaadin documentation on styling applications].
...
In Vaadin, a stylesheet can be https://vaadin.com/docs/v25/styling/advanced/reusable-theme[made reusable] by creating a CSS in a folder with the name of the theme.
...
These CSS custom properties represent colors, fonts, sizes and other styles, which are used in various components.
The style properties can be modified to change the styling either globally, or in a certain scope.

As linkki theme is based on the Lumo theme, all link:https://vaadin.com/docs/v25/styling/themes/lumo/lumo-style-properties[Lumo style properties] are also available.
...
Success colors are used to display result messages of an successful operation, or messages that indicate that a process can be continued.
They are used to display validation messages, e.g. on fields or in notifications.
...
Error color indicates that a process cannot be started or continued.
They are used to display validation messages,e.g. on fields or in notifications
...
https://vaadin.com/docs/v24/styling/lumo/lumo-style-properties/typography#lumo-font-family[Lumo default font family] is modified to make sure that Segoe UI as system font is preferred over Robot font.
