...
image::{images}99_tutorial/eclipse_select_archetype.png[]
...
image::{images}99_tutorial/eclipse_maven_project.png[]

*3. Generating the project in IntelliJ*

Alternatively, when using IntelliJ you can use the wizard for new Maven Projects.

Start the wizard by selecting File - New - Project. Select the Maven Archetype Generator, give it a name and set the Catalog to Maven Central. In the Archetype selectbox you can search for linkki and select the appropriate Archetype from the list. Finally, choose the version and set the Property `ApplicationName` to "BusinessPartner".

.Generate project in IntelliJ
image::{images}99_tutorial/intellij_maven_project.png[]
...
image::{images}99_tutorial/initial_project_structure.png[width=426]
...
To get a better understanding of the classes this archetype creates, refer to the section {linkkidoc}/07_application-framework/02_application_layout.html[Application Layout] in the chapter Application Framework of the *linkki* Documentation.
...
image::{images}99_tutorial/example_application.png[]
...
image::{images}99_tutorial/empty_application.png[]
