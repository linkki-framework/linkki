...
Showing an additional icon with the component's caption, the `@BindIcon` annotation can be used.

NOTE: Only <<ui-button, UIButton>> and <<ui-link, UILink>> are currently supported. The `@BindIcon` will override an icon, that has already been set by the @UI-annotation.
