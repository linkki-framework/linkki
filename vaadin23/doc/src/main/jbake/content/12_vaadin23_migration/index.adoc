:jbake-title: Vaadin 8 to 23 Migration
:jbake-type: chapter
...
[[vaadin23-migration-applicationLayout]]
ApplicationLayout::
+
[cols="a,a"]
|===
...
| * Create a new sub-class of `ApplicationLayout` (e.g. `MyApplicationLayout`) 
  * In the new subclass, pass an instance of the customized `ApplicationConfig` to the super constructor. This `ApplicationConfig` was previously used in the implementation of <<vaadin23-migration-linkkiUi, `LinkkiUi`>>.
...
| Move the implementation of `#createApplicationLayout` to the implementation of <<vaadin23-migration-applicationLayout, `ApplicationLayout`>>.
...
If additional information needs to be stored to be displayed in the header and footer, follow the migration steps for the <<vaadin23-migration-applicationHeader, `ApplicationHeader`>> and  the <<vaadin23-migration-applicationFooter, `ApplicationFooter`>>.
...
| * If `getHeaderDefinition` was not overridden, no migration is needed.
  * If a custom implementation of `ApplicationHeader` was used, consult migration steps of the <<vaadin23-migration-applicationHeader, `ApplicationHeader`>>.
...
| * If `getFooterDefinition` was not overridden, no migration is needed.
  * If a custom implementation of `ApplicationFooter` was used, consult migration steps of the <<vaadin23-migration-applicationFooter, `ApplicationFooter`>>.
|===

[[vaadin23-migration-applicationHeader]]
ApplicationHeader::
...
| See migration steps of the <<vaadin23-migration-applicationMenuItemDefinition, ApplicationMenuItemDefinition>>.
|===

[[vaadin23-migration-applicationMenuItemDefinition]]
...
* with name, id and a `Class` to navigate to. The given class should be a <<vaadin23-migration-views, route component>>. 
...
[[vaadin23-migration-applicationFooter]]
ApplicationFooter::
