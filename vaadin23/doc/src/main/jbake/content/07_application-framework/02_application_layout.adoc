...
`MenuItems` for theme variants can be added using the method `addThemeVariantToggles(MenuItem, ThemeVariantToggleMenuItemDefinition...)` when overriding `addHelpMenuItems(MenuItem)` in your subclass of `ApplicationHeader`.
...
protected void addHelpMenuItems(MenuItem helpMenu) {
    ThemeVariantToggleMenuItemDefinition menuItem = new ThemeVariantToggleMenuItemDefinition("Custom Theme", "appmenu-theme-custom-theme-id", "custom-theme-variant")
    addThemeVariantToggles(helpMenu, menuItem);
}
----
