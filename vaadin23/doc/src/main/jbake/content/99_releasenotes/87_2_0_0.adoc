...
It is now possible to set IDs for `ApplicationMenuItems`. All constructors of `ApplicationMenuItemDefinition` were extended with a parameter for the ID. The old constructors have been deprecated. If they are still used the ID is derived from the name of the `ApplicationMenuItem` and converted to an appripriate format with the prefix `appmenu-` (see <<application-menu-item-definition, documentation>>).
