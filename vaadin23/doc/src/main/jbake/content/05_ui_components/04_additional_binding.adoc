...
TIP:  BindSuffix can add a Suffix to UI Elements which supports the HasPrefixAndSuffix interface. Common UI Elements which supports `BindSuffix` are UIIntegerField, UIDoubleField, UIDecimalField, UITextField, UITextArea, UICustomField.