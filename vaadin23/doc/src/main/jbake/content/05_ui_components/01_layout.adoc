...
The layout for all the components within a `UISection` can be defined with the attribute `layout`. *linkki* defines the possible layouts in the enum `SectionLayout`: HORIZONTAL or COLUMN.
.
...
Column Section Layout:: With the column layout fields are displayed one below the other. Labels are displayed on the left of the component.
