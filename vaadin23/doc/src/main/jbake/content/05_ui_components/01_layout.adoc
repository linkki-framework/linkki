...
Horizontal Section Layout:: With the horizontal layout all input fields are displayed on a line, side by side in the order of their position. Labels are displayed on top of the component. The attribute `columns` is ignored. `UITextField` and `UITextArea` take up the same space as all other components by default. In addition, components are wrapped and moved into a new line if screen width is decreased. 
