...
[[ui-datefield]]
==== UIDateField
The annotation `@UIDateField` generates a date input field and corresponds to `com.vaadin.flow.component.datepicker.DatePicker`. It allows specifying a date format using the property `dateFormat: String`. If not date format is defined the format matching the `Locale` setting is used. *linkki* uses `"dd.MM.yyyy"` by default for German. For other languages `DateFormat.SHORT` from the JDK is used.
