...
[source, java]
----
include::{source-dir-binding-sample}/java/org/linkki/samples/binding/pmo/ContactSectionPmo.java[tags=radiobutton]
----
...
[source, java]
----
include::{source-dir-binding-sample}/java/org/linkki/samples/binding/annotation/UIRadioButtonGroup.java[tags=custom-annotation]
----
...
[source, java]
----
include::{source-dir-binding-sample}/java/org/linkki/samples/binding/annotation/UIRadioButtonGroup.java[tags=component-definition]
----
...
The property on the model side (PMO / model object) that is considered for data binding is called `BoundProperty`. In our example the property "gender" inside the PMO that is annotated with `@UIRadioButtonGroup` is the `BoundProperty` for this specific `RadioButtonGroup`.
A `BoundProperty` needs to have at least the property name of the PMO property. Optionally, it can contain the model object and attribute for model binding.

The next listing shows the `UIRadioButtonGroupBoundPropertyCreator` that returns the `BoundProperty`:

.Implementation of the UIRadioButtonGroupBoundPropertyCreator
[source, java]
----
include::{source-dir-binding-sample}/java/org/linkki/samples/binding/annotation/UIRadioButtonGroup.java[tags=bound-property]
----

The `UIRadioButtonGroupBoundPropertyCreator` utilizes a factory method to create the `BoundProperty` passing the annotated method element as required property. Afterwards it adds the model object and the model attribute to the `BoundProperty`. By following this mechanism, the custom UI element annotation now supports the binding to the PMO and the Domain Model as described in <<domain-model-binding, Binding to the Domain Model (Data on Multiple Layers)>>.
...
[source, java]
----
include::{source-dir-binding-sample}/java/org/linkki/samples/binding/annotation/UIRadioButtonGroup.java[tags=aspect-definition]
----
