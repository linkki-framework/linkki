...
public class ReadOnlyBehaviorPmo {
    ...
    public void disabledOnReadOnly() {
        Notification.show("You just clicked on the button that will be disabled when the application is in read-only mode");
    }

    @BindReadOnlyBehavior(ReadOnlyBehaviorType.INVSIBLE)
    @UIButton(position = 20, caption = "@BindReadOnlyBehavior(INSIVISLBE)")
    public void invisibleOnReadOnly() {
        Notification.show("You just clicked on the button that will be invisible when the application is in read-only mode");
    }
