...
If your data is hierarchical (a tree-like structure with parent-child-relationships between rows, for example when summarizing values over certain categories), the row PMO for the <<ui-container-pmo,ContainerPmo>> should implement org.linkki.core.ui.table.HierarchicalRowPmo<ROW>; If you do, the resulting table will be a `TreeTable` which allows hiding and showing the child-rows of a row.

image::{images}{images-folder-name}/treetable.png[A hierarchical table,align="center"]

[source, java]
----
include::{source-dir-treetable}/org/linkki/samples/treetable/fixed/pmo/PersonRowPmo.java[tags=hierarchical-row-pmo]
----
