...
`@BindReadOnlyBehavior` changes a component's behaviour when it is set to read-only. The read-only status is determined by the <<property-dispatcher, property-dispatcher-chain>>. Therefore there is no need to define additional methods as is usually the case with dynamic aspect definitions. 

TIP: This annotation is particularly useful for components that don't have a native read-only state, such as buttons.
...
|===
|`INVISIBLE`   |Component is invisible in read-only-mode
