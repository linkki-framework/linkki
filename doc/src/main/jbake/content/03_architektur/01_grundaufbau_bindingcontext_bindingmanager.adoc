...
To create and manage one or more `BindingContexts`, a `BindingManager` is used. For a given name (for instance derived from a class) it returns a `BindingContext`, creating a new one when necessary. The desired binding behavior can be defined in subclasses of `BindingManager` (see <<vertikales-binding,Vertical Binding>>). If no special behavior needs to be implemented, a `DefaultBindingManager` can be used. A `BindingManager` is typically created `@ViewScoped` and passed to each `Page` (see <<page,Page>>).
