...
Table classes can now implement the `SelectableTable` interface, and receive callbacks when the user selects a table row. Updating the current selection programmatically is also possible.