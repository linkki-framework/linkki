...
If a PMO property has no `modelAttribute` set, *linkki* searches for a modelAttribute with the same name in the model object and uses it if present. Previously, *linkki* was also using the setter of the property in the model object even if the PMO property has no setter. Since it can be the desired behaviour to not have an getter (for example in read only situations), a setter method for the the PMO property is now required for data binding to the model object (push).
