...
In previous *linkki* versions, the most common way to disable or hide components in read-only-mode was to determine the value for those aspects manually, often by passing business-objects as parameters. With this new annotation, a component will automatically change to either disabled or invisible when it is set to read-only. Use the values `ReadOnlyBehaviorType.INVISIBLE` or `ReadOnlyBehaviorType.DISABLED`, respectively. Further technical Details can be found in the <<bind-read-only-behavior, documentation>>.
