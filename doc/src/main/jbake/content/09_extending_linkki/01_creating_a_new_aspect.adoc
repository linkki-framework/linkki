...
Lastly, `BindTooltip` has to be annotated with `@LinkkiAspect` to tell *linkki* how to create the aspect from the annotation. The value for that annotation is a class implementing `LinkkiAspect.Creator<BindTooltip>` that can be created as an inner class:
