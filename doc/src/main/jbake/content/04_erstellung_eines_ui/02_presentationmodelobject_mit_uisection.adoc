...
The <<ui-element, `UI elements`>> are bound either to the 'properties' of a <<binding-modelObjet, `ModelObject`>> or a PMO. A detailed description can be found in the chapter <<domain-model-binding, `Data on Multiple Layers`>>.

TIP: In all cases, if the `ModelObject` or PMO doesn't have a corresponding setter, the field is `readOnly`.

In the <<abschnitt-form-binding, first section>> the creation of a section for a 'standard' form is exemplified. In the <<abschnitt-binding-table, second section>> the creation of a section with a table is detailed.
...
The so called 'Row PMO' is a regular PMO as described in <<abschnitt-form-binding, the first section>>. The only difference is that the UI elements are also annotated with <<ui-table-column, `@UITableColumn`>>.
