...
* Existing `LinkkiAspectDefinitions` can no longer be used in `@LinkkiAspect` directly. It requires an implementation of `LinkkiAspect.Creator` to create the aspect definitions. +
+
...
[source,java]
----
public class MyAspectDefinitionCreator implements LinkkiAspect.Creator<@NonNull MyAnnotation> {
