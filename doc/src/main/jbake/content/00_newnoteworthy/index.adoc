...
With the new method `open()` in `OkCancelDialog`, a dialog can be directly opend by calling `dialog.open()` instead of using `PmoBasedDialogFactory.open(dialog)`. In addition, the `open()` method can now be extended to add additional behaviors to the dialog. The `open(OkCancelDialog)` method in `PmoBasedDialogFactory` is thus deprecated.
