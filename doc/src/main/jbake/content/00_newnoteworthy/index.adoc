...
The right `MenuBar` of the `ApplicationHeader` is now equipped with a `HelpMenu` which provides an `ApplicationInfoDialog` by default. It is customizable implementing `ApplicationConfig` and extending `ApplicationHeader` and `ApplicationInfoPmo`.

==== Footer

The `ApplicationFooter` is now defined in `ApplicationConfig` as optional. In order to hide it, override `ApplicationConfig#getFooterDefinition()`.

==== Customized sample application

A custom sample application is implemented with:

* no `StartMenuItemDefinition`,
* customized `ApplicationInfoDialog`,
* hidden `ApplicationFooter`,
* another help `MenuItem`.
...
==== *linkki* is getting independent of cdi
