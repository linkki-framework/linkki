...
==== `ContainerPmo` as bound object of @UITableColumn attributes
...
We made a major refactoring concerning the internal handling of tables and columns. All properties of a table column (like label, width, expand ratio, collapsible) are now handled using <<creating-aspects,linkki aspects>>. With this refactoring these properties are real bindings now. Although these bindings are not dynamic (at the moment) we decided to use the `ContainerPmo` as bound object. That means:
  ...
  * If you create (or we in the future) a dynamic aspect binding the corresponding method must be defined in the `ContainerPmo` instead of in the PMO of the row.

WARNING: If you are translating the label of the columns with `linkki-messages.properties` files, you must change the existing translation keys to the class name of the container PMO. +
Assume that there is an entry with the key `TestRowPmo_name_label` for the property `name` in `TestRowPmo` which is used in `TestTablePmo` as row PMO. This key be changed to `TestTablePmo_name_label`.
...
TIP: `getFooterPmo` is now bound fully dynamically. This fixes a previous bug that the footer is not removed dynamically once created.

==== Aspect for labels

The label is not bound using aspects. If you have created a custom component that does *not* use the common `FieldAspectDefinition` you might need to add the new `LabelAspectDefinition` to your component.
