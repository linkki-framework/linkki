...
| **SETTER_ONLY_IN_MODEL_OBJECT** | Reports that a property with corresponding bound model attribute has a getter but no setter in the PMO although there is a setter in the model object. This is a legal case but behavior changed with *linkki* 1.1 (see <<nn-setter-in-mo, New&Noteworthy: Setter calls in model object>> ). | IGNORE
|===
