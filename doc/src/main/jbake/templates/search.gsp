        ...
        <div class="page-wrapper" tabindex="-1" role="main">
            <div class="page-inner">             
                <section class="normal markdown-section">
                    ...
                    def ankorMap = new HashMap<String, File>()
                    ...
                    for (File dirAdoc : basedir.listFiles()) {
                        for (File adoc : dirAdoc.listFiles()) {
                            ...
                            for (String line : buffLines) {
                                def matcher = line.toString() =~ /^\[\[.*?\]\]/
                                while (matcher.find()) {
                                    String s = matcher.group();
                                    (adoc.toPath().toString() =~ /.*\/(.*).adoc/).each{all, filename ->
                                        ankorMap.put(s.substring(2, s.length() - 2), targetDir + "/" + dirAdoc.getName() + "/" + filename)}
                                }
                            }
                        }
                    }
                    ...
                    for (File dirTarget : nonInternalDirs) {
                        for (File html : dirTarget.listFiles()) {
                            ...
                            result = sourceCode.replaceAll(/(href=\")(#)(.*?)\"/, {all, href, hash, ankorKey ->
                                File absolutePath = ankorMap.get(ankorKey);
