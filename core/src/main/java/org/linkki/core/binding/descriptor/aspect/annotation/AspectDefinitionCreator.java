...
/**
 * Implementations of this interface describe how a {@link LinkkiAspectDefinition} is created from
 * the context of an {@link Annotation}. The implementation class is defined in
 * {@link LinkkiAspect#value()}.
 * 
 * @implSpec The implementation must have a default constructor which will be called while reading
 *           the annotations of a PMO class.
 */
public interface AspectDefinitionCreator<T extends Annotation> {

    /**
     * Creates the aspect definition from the annotation that is annotated with
     ...
     * 
     * @implSpec The {@link LinkkiAspectDefinition} is instantiated for every property. That means
     *           it is valid to store the given annotation in a field.
     */
