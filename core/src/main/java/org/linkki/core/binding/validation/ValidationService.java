...
public interface ValidationService {
    ...
    /**
     * Message code for a {@link org.linkki.core.binding.validation.message.Message Message}
     * indicating a non-specific fatal validation error.
     */
    ...
    /**
     * Returns the validation messages returned by {@link #getValidationMessages()} after applying
     * the {@link #getValidationDisplayState() ValidationDisplayState's}
     * {@link ValidationDisplayState#filter(MessageList) filter(MessageList)} method to them.
     * 
     * @implNote As {@link #getValidationDisplayState()} returns
     *           {@link ValidationDisplayState#SHOW_ALL} by default, the {@link MessageList} is
     *           returned unfiltered unless {@link #getValidationDisplayState()} is overridden by an
     *           implementation.
     * 
     * @return a {@link MessageList} containing all validation messages that match the
     *         {@link ValidationDisplayState} or an empty {@link MessageList} if there are no
     *         messages at all
