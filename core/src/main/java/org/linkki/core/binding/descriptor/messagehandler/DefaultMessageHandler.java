...
/**
 * The default message handler for all components. It uses the {@link PropertyDispatcher} to retrieve
 * the messages which are relevant for the specific binding. If there are any messages they will be set
 * to the component and mark the component as invalid using
 * {@link ComponentWrapper#setValidationMessages(MessageList)}.
 * <p>
 * If there is no specific message handler configured for a component, this message handler will be used
 * as default.
 */
public class DefaultMessageHandler implements LinkkiMessageHandler {
    ...
    /**
     * Selects the messages that are relevant for this specific binding. The default implementation uses
     * the given {@link PropertyDispatcher} but the behavior could be overwritten to select other or
     * additional messages.
     * 
